...
export default function ScenarioSelector({ scenarios, activeCharacter, showCountdown: initialShowCountdown, partyCharacters = [], partySlots = [], currentUserId = '', roomCode: initialRoomCode = null, isHost = false }: ScenarioSelectorProps) {
  ...
  const totalVotesCast = Object.values(scenarioVotes).reduce((a, b) => a + b.length, 0) + regenerationVotes;
  ...
  const handleTiebreakerDiceRoll = async () => {
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        ...
        showToast('Dice rolling initialization timed out. Please try again.', 'error');
      } else {
  ...
  const getWinningScenarioFromDiceRoll = useCallback(() => {
    ...
    if (!winnerVote) {
      console.warn('[SCENARIO SELECTOR] Dice winner did not cast a vote');
      return null;
    }
  ...
  }, [diceState, currentScenarios, userVotes]);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (voteUpdates.length > 0) {
            // Use voteCounts if available, otherwise calculate from votes
            const updatedVotes = data.voteCounts || {};
            if (!data.voteCounts) {
              voteUpdates.forEach((vote: ScenarioVote) => {
                updatedVotes[vote.scenarioId] = (updatedVotes[vote.scenarioId] || 0) + 1;
              });
            }
            console.log(`[INITIAL VOTES] Updated scenarioVotes:`, updatedVotes);
            ...
            console.log(`[INITIAL VOTES] Updated userVotes:`, updatedUserVotes);
            ...
            if (scenariosResponse.ok) {
              ...
              if (updatedScenarios.length > 0) {
                ...
                const allVoteCounts = updatedScenarios.map((s: AdventureScenario) => updatedVotes[s.id] || 0);
                const maxVotes = Math.max(...allVoteCounts, 0);
                const tiedScenarios = updatedScenarios.filter((s: AdventureScenario) => (updatedVotes[s.id] || 0) === maxVotes);
  ...
  }, [initialRoomCode, adventureStarted]); // Removed showDiceRoll dependency
  ...
  const allVoteCounts = currentScenarios?.map(s => scenarioVotes[s.id] || []) || [];
  const maxVotes = Math.max(...allVoteCounts.map(votes => votes.length), 0);
  const tiedScenarios = currentScenarios?.filter(s => (scenarioVotes[s.id]?.length || 0) === maxVotes) || [];
  const hasClearWinner = tiedScenarios.length === 1 && maxVotes > 0;
  ...
  useEffect(() => {
    console.log('Tiebreaker debug:', {
      ...
      totalActiveSlots,
      totalVotesCast,
      tiedScenarios: currentScenarios?.filter(s => (scenarioVotes[s.id]?.length || 0) === maxVotes).map(s => ({ id: s.id, title: s.title, votes: scenarioVotes[s.id]?.length || 0 }))
    });
  }, [allHaveVoted, regenerateMajority, hasClearWinner, maxVotes, needsTiebreaker, scenarioVotes, userVotes, currentScenarios, totalActiveSlots, totalVotesCast]);
  ...
  useEffect(() => {
    if (needsTiebreaker && !showDiceRoll && !isInitializingDice) {
      ...
      console.log('[TIEBREAKER] Debug:', { allHaveVoted, regenerateMajority, hasClearWinner, maxVotes, needsTiebreaker, scenarioVotes, userVotes });
      setShowDiceRoll(true);
    }
  }, [needsTiebreaker, showDiceRoll, isInitializingDice, allHaveVoted, regenerateMajority, hasClearWinner, maxVotes, scenarioVotes, userVotes]);
  ...
  useEffect(() => {
    if (allHaveVoted && maxPossibleVotes > 0 && !regenerateMajority && !hasClearWinner) {
      ...
      console.log('[TIEBREAKER] Debug:', { allHaveVoted, regenerateMajority, hasClearWinner, maxVotes, maxPossibleVotes, needsTiebreaker, scenarioVotes, userVotes });
      if (!showDiceRoll && !isInitializingDice) {
        setShowDiceRoll(true);
      }
    }
  }, [allHaveVoted, regenerateMajority, hasClearWinner, maxVotes, maxPossibleVotes, showDiceRoll, isInitializingDice, scenarioVotes, userVotes, needsTiebreaker]);
  ...
  }, [initialRoomCode, adventureStarted, allHaveVoted, regenerateMajority, hasClearWinner, maxVotes, maxPossibleVotes, showDiceRoll, isInitializingDice, scenarioVotes, userVotes, needsTiebreaker]);
      ...
      <div className="w-full max-w-4xl bg-black bg-opacity-70 p-8 rounded-lg border border-gray-700 shadow-lg text-white">
        ...
        {isGenerating ? (
          ...
          <div className="mt-8">
                    ...
                    {partySlots.length > 1 && (
                      <div className="mt-2 space-y-1">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-gray-400">Votes: {voteCount.length}</span>
                        ...
                        <div className="text-xs text-gray-400">
                          {scenarioVotes[scenario.id]?.length > 0 ? (
                            scenarioVotes[scenario.id]?.map((vote: ScenarioVote, index: number) => {
                              ...
                              return (
                                <span key={index} className={`inline-block px-2 py-1 rounded mr-1 mb-1 ${
                                  ...
                                  {vote.userId} (Slot {vote.slotIndex}) • {timeAgo}
                                </span>
                              );
                            })
                          ) : (
              ...
              <div className="mt-8 text-center">
                ...
                {winningScenario && (
                  <div className="mt-4 p-4 bg-green-900 bg-opacity-30 rounded-lg border border-green-600">
                    ...
                    <p className="text-sm text-green-400 mt-1">With {scenarioVotes[winningScenario.id]?.length || 1} vote(s)</p>
                  </div>
                )}
              </div>
            )}
            ...
            {diceState && needsTiebreaker && showDiceRoll && (
              <div className="mt-8" style={{ zIndex: 10000, backgroundColor: 'transparent' }}>
                  ...
                  {diceState && diceState.players.length > 0 && (
                    <div className="mt-4 bg-yellow-900 bg-opacity-30 rounded-lg p-4 inline-block">
                      ...
                      <div className="text-yellow-200 text-sm">
                        {diceState.players.map((player) => {
                          const votedScenario = currentScenarios?.find(s => s.id === userVotes[player.slotIndex]);
                          return votedScenario ? votedScenario.title : 'No vote';
                        }).filter((title, index, arr) => arr.indexOf(title) === index).join(' vs ')}
                      </div>
                    </div>
                  )}
                </div>
                ...
                {!diceState && !isInitializingDice && showDiceRoll && (
